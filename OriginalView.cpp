...
void OriginalView::draw()
{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		Point source(cursor.x + scrollpos.x, startrow + drawHeight - cursor.y );
		if (0 < source.x && source.x < m_pDoc->m_nWidth && 0 < source.y && source.y < m_pDoc->m_nHeight)
		{
			glPointSize(5.0f);
			glBegin(GL_POINTS);
				glColor3ub(0xFF, 0, 0);
...
void OriginalView::setCursor(Point cs)
{
	cursor = cs;
	refresh();
}
